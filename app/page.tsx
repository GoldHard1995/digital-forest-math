...
type View = 'map' | 'question' | 'second-question' | 'teacher' | 'assign' | 'report';
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      {view === 'map' && <StudentMap onStartFirst={() => setView('question')} onStartSecond={() => setView('second-question')} profile={profile} demoMode={demoMode} />}
...
function StudentMap({ onStartFirst, onStartSecond, profile, demoMode }: { onStartFirst: () => void; onStartSecond: () => void; profile: Profile | null; demoMode: boolean }) {
  return (
        ...
        <section aria-label="森林地圖" className="map-panel relative min-h-[470px] overflow-hidden rounded-[28px] border border-forest/15 p-6 shadow-sm">
          ...
          <div className="relative z-10 grid grid-cols-5 gap-3 pt-14">
            {stages.map((stage, index) => (
              <button key={stage.title} type="button" disabled={stage.status === 'locked'} onClick={stage.status === 'active' ? onStartFirst : stage.status === 'available' ? onStartSecond : undefined} className={`stage ${stage.status}`} aria-label={`${stage.title}，${stage.subtitle}`}>
                ...
                {index === 1 && <span className="mt-3 inline-flex rounded-full bg-forest px-2 py-1 text-[10px] font-bold text-cream">試玩</span>}
              </button>
            ))}
          </div>
