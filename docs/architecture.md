...
- `app/page.tsx`：單頁低擬真原型，包含登入、角色分流、學生地圖、第一至第四關及小頭目答題、教師原型。功能增加後應按領域逐步拆分，不應一次重寫整個檔案。
...
- `lib/fourth-stage-content.ts`：第四關「異號加法」已核准的本機內容；A 部分使用個位數，B 部分使用兩位數的兩項相加，C 部分使用三個有向數相加。目前它是審核中的來源，不是正式資料庫題庫。
- `lib/boss-stage-content.ts`：小頭目「聚能獸」已核准的 `10` 題加法挑戰；A、B、C 部分按 `4／4／2` 分層，使用整數答案。目前它是審核中的來源，不是正式資料庫題庫。
