...
- 現階段：已建立可登入的低擬真原型；第一關 `13` 題內容已由教師核准並作本地提交（`9a068dc`），第二關「比較大小」及第三關「同號加法」各 `13` 題亦已由教師核准並分別作本地提交（`7d4edfb`、`223dbf7`）；三關均尚未推送至 `GitHub` 或部署，正式作答資料及教師報表仍未接駁。
...
目前工作樹沒有未提交改動。第三關實作及同步文件已於本機提交於 `223dbf7`；第二關實作及同步文件已於本機提交於 `7d4edfb`；第一關改動已於本機提交於 `9a068dc`。三者均尚未推送至 `GitHub`，亦未部署至正式網址。
...
1. 在有實體裝置時完成第二關拖拉及第三關數字鍵盤於 `iPadOS 15 Safari` 的橫屏驗證，並記錄結果。
2. 依教師要求先建立第一區其餘五項技能；每項題目先列出供核准，不發布未核准題目。
